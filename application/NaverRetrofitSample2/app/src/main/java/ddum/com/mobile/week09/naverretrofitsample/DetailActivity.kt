...
class DetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
            ...
            Glide.with(this@DetailActivity)
                ...
                .into( object: CustomTarget<Bitmap>(350, 350) {
                    override fun onResourceReady(
                        ...
                        val imageFile = File("${filesDir}/images", "image${FileManager.getImageFileName(
                            imageUrl.toString()
                        )}.jpg")
                    ...
                    override fun onLoadCleared(placeholder: Drawable?) {
                        Log.d(TAG, "Error occured while loading image!")
                    }
                })
        }
        ...
        detailBinding.btnRemove.setOnClickListener {
            val deleteFile = File(filesDir, "/images/image${
                FileManager.getImageFileName(
                    imageUrl.toString()
                )
            }.jpg")
            deleteFile.delete()
